...
func selectKernels() batchKernels {
	k := batchKernels{add: add16Scalar, mul: mul16Scalar, addWrap: add16WrapScalar}
...
// add16WrapAVX2 is the speed ceiling for the add benchmarks: a pure vector sum
// with no overflow work at all.
func add16WrapAVX2(dst, a, b []Q16) {
	const lanes = 8
	rd, ra, rb := rawInt32(dst), rawInt32(a), rawInt32(b)
	i := 0
	for ; i+lanes <= len(ra); i += lanes {
...
// axpyClampFusedAVX2 keeps every intermediate of the composite kernel in a
// register, so one pass over memory does all four operations.
func axpyClampFusedAVX2(x, m, v []Q16, bias, lo, hi Q16) {
	const lanes = 8
	rx, rm, rv := rawInt32(x), rawInt32(m), rawInt32(v)
	biasV := archsimd.BroadcastInt32x8(bias.raw)
	loV := archsimd.BroadcastInt32x8(lo.raw)
	hiV := archsimd.BroadcastInt32x8(hi.raw)

	var events uint64
	i := 0
	for ; i+lanes <= len(rx); i += lanes {
		s, e1 := vecAddSat(archsimd.LoadInt32x8(rv[i:]), biasV)
		p, e2 := vecMulSat(archsimd.LoadInt32x8(rm[i:]), s)
		t, e3 := vecAddSat(archsimd.LoadInt32x8(rx[i:]), p)
		t.Max(loV).Min(hiV).Store(rx[i:])
		events += e1 + e2 + e3
	}
	if events != 0 {
		saturationEvents.Add(events)
	}
	axpyClampFused(x[i:], m[i:], v[i:], bias, lo, hi)
}
